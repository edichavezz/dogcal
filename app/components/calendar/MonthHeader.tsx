...
export default function MonthHeader() {
  ...
  if (isMobileView) {
    ...
    const windowIncludesToday = [mobileFocusDate, addDays(mobileFocusDate, 1), endDate].some(
    ...
    return (
      <div className="flex items-center justify-between px-4 py-3 bg-white border-b border-slate-200">
        ...
        <button onClick={goToToday} className="flex flex-col items-center" aria-label="Go to today">
          <h2 className="text-lg font-semibold text-slate-900 font-display">{rangeLabel}</h2>
          {!windowIncludesToday && (
