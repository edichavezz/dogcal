...
type SuggestionPreviewCardProps = {
  ...
  onClick?: () => void; // If provided, renders as a clickable button
  interactive?: boolean; // If false, renders as a non-clickable div (default true)
};
...
export default function SuggestionPreviewCard({
  ...
  const baseClassName = 'block w-full bg-white rounded-xl sm:rounded-2xl p-3 sm:p-4 border-2 border-dotted border-blue-300 transition-all hover:shadow-lg hover:border-blue-400';
  ...
  if (onClick) {
    return (
      <button type="button" onClick={onClick} className={`text-left ${baseClassName}`}>
        {inner}
      </button>
    );
  }
  ...
  return (
    <Link href="/approvals" className={baseClassName}>
      {inner}
    </Link>
  );
}
